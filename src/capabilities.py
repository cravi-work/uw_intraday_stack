...
def extract_db_truth(con: duckdb.DuckDBPyConnection) -> Dict[str, Any]:
    ...
    query = """
        SELECT e.ticker, d.method, d.path, e.last_attempt_ts_utc, e.last_success_ts_utc,
               e.last_change_ts_utc, e.last_attempt_http_status, e.last_attempt_error_type,
               e.last_attempt_error_msg
        FROM endpoint_state e JOIN dim_endpoints d ON e.endpoint_id = d.endpoint_id
    ...
    for row in rows:
        (ticker, method, path, attempt_ts, success_ts, change_ts, http_status, err_type, err_msg) = row
        ticker = str(ticker).upper()
        if ticker not in results: results[ticker] = {"endpoints": {}}
        ...
        if success_age is not None and attempt_age is not None:
            if (http_status is not None and 200 <= http_status < 300) and (err_type is None):
                if change_age is not None and change_age > 5400: # 90-min SLA
                    health_status = "STALE"
                else:
...
def check_capabilities(catalog_path: str, db_path: str, plan_path: str, output_format: str = "text") -> None:
    ...
    try:
        ...
        if output_format == "json": print(json.dumps(truth_data, indent=2))
        else: print_text_report(truth_data)
    finally:
        con.close()