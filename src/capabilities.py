...
def extract_db_truth(con: duckdb.DuckDBPyConnection) -> Dict[str, Any]:
    ...
    for row in rows:
        ...
        if success_age is not None and attempt_age is not None:
            # We consider an endpoint currently healthy if its last attempt was successful
            # (i.e. success age is the same as attempt age, or it hasn't failed since last success)
            is_healthy = (http_status is not None and 200 <= http_status < 300) and (err_type is None)
...
def print_text_report(truth_data: Dict[str, Any]) -> None:
    ...
    for ticker, data in truth_data.items():
        ...
        for ep_key, stats in endpoints.items():
            status_mark = "✅" if stats["is_healthy"] else "❌"
