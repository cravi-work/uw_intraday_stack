...
class DbWriter:
        ...
        *, 
        ...
        con.execute(
            ...
            INSERT INTO snapshots (
                ...
                market_close_utc, post_end_utc, seconds_to_close, created_at_utc
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
            """, 
            [
                str(snapshot_id), str(run_id), asof_ts_utc, ticker.upper(), session_label, 
                is_trading_day, is_early_close, data_quality_score, market_close_utc, 
                post_end_utc, seconds_to_close, datetime.now(UTC)
            ]
        )
    ...
    def insert_prediction(self, con, p):
        ...
        con.execute(
            ...
            AND horizon_minutes IS NOT DISTINCT FROM ? 
            AND horizon_seconds IS NOT DISTINCT FROM ?
            """, 
        ...
        con.execute(
            ...
            [
                ...
                p.get("start_price"), p.get("bias"), p["confidence"], p["prob_up"], 
                p["prob_down"], p["prob_flat"], p["model_name"], p["model_version"], 
                p["model_hash"], p.get("is_mock", False), json.dumps(p.get("meta_json", {}))
            ]
        )
        return pid
    ...
    def upsert_endpoint(self, con, method, path, params, registry):
        ...
        con.execute(
            ...
            [eid, method, path, sig, registry.params_hash(params), json.dumps(params)]
        )
        return eid
    ...
    def insert_raw_event(self, con, run_id, ticker, endpoint_id, req_at, rec_at, status, lat, ph, pj, retry, etype, emsg, circ):
        ...
        con.execute(
            ...
            INSERT INTO raw_http_events (
                event_id, run_id, requested_at_utc, received_at_utc, ticker, endpoint_id, 
                http_status, latency_ms, payload_hash, payload_json, is_retry, 
                error_type, error_msg, circuit_state_json
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """, 
            [
                str(eid), str(run_id), safe_req, safe_rec, ticker, endpoint_id, status, 
                lat, ph, json.dumps(pj) if pj else None, retry, etype, emsg, 
    ...
    def begin_run(self, con, asof_ts_utc, session_label, is_trading_day, is_early_close, config_version, api_catalog_hash, notes=""):
        ...
        con.execute(
            ...
            """, 
            [
                str(run_id), datetime.now(UTC), None, asof_ts_utc, session_label, 
                is_trading_day, is_early_close, config_version, api_catalog_hash, notes
            ]
        )
        return run_id
    ...
    def upsert_endpoint_state(
        ...
        con.execute(
            ...
            """,
            [
                str(event_id), attempt_ts_utc, res.status_code, res.error_type, 
                res.error_message, ticker, endpoint_id
            ]
        )
        ...
        if is_success_class:
            con.execute(
                """
                UPDATE endpoint_state SET 
                    last_success_event_id=?, last_success_ts_utc=?, last_payload_hash=?,
                ...
                """,
                [
                    str(event_id), attempt_ts_utc, res.payload_hash, 
                    changed, attempt_ts_utc, changed, str(event_id), 
                    ticker, endpoint_id
                ]
            )
    ...
    def insert_features(self, con, snapshot_id, feats, meta):
        for k, v in feats.items(): 
            con.execute(
                ...
                [str(snapshot_id), k, v, json.dumps(meta)]
            )
    ...
    def insert_lineage(
        ...
        con.execute(
            ...
            """, 
            [
                str(snapshot_id), endpoint_id, str(used_event_id) if used_event_id else None, 
                freshness_state, data_age_seconds, payload_class, na_reason, 
    ...
    def end_run(self, con, run_id):
        con.execute(
            "UPDATE meta_runs SET ended_at_utc=? WHERE run_id=?", 
            [datetime.now(UTC), str(run_id)]
        )
