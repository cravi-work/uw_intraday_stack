...
class DbWriter:
    ...
    def insert_raw_event(self, con, run_id, ticker, endpoint_id, req_at, rec_at, status, lat, ph, pj, retry, etype, emsg, circ):
        ...
        con.execute(
            ...
            [
                str(eid), str(run_id), safe_req, safe_rec, ticker, 
                endpoint_id, status, lat, ph, json.dumps(pj) if pj is not None else None, 
                retry, etype, emsg, json.dumps(circ) if circ else None
            ]
        )
        return eid
    ...
    def insert_lineage(self, con, snapshot_id, endpoint_id, used_event_id, freshness_state, data_age_seconds, payload_class, na_reason, meta_json):
        con.execute(
            """INSERT INTO snapshot_lineage (snapshot_id, endpoint_id, used_event_id, freshness_state, data_age_seconds, payload_class, na_reason, meta_json) 
               VALUES (?,?,?,?,?,?,?,?) 
               ON CONFLICT (snapshot_id, endpoint_id) 
               DO UPDATE SET used_event_id = excluded.used_event_id, freshness_state = excluded.freshness_state, data_age_seconds = excluded.data_age_seconds, payload_class = excluded.payload_class, na_reason = excluded.na_reason, meta_json = excluded.meta_json""", 
