...
import contextlib, hashlib, json, os, uuid, logging
...
class DbLockError(RuntimeError): pass
...
class DbWriter:
    ...
    def upsert_endpoint(self, con, method, path, params, registry):
        ...
        con.execute("""INSERT INTO dim_endpoints (endpoint_id, method, path, signature, params_hash, params_json) VALUES (?,?,?,?,?,?)""", 
    ...
    def begin_run(self, con, asof_ts_utc, session_label, is_trading_day, is_early_close, config_version, api_catalog_hash, notes=""):
        run_id = uuid.uuid4()
        con.execute("""INSERT INTO meta_runs (run_id, started_at_utc, ended_at_utc, asof_ts_utc, session_label, is_trading_day, is_early_close, config_version, api_catalog_hash, notes) 
                       VALUES (?,?,?,?,?,?,?,?,?,?)""", 
