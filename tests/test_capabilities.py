...
def setup_db(db_path: str) -> duckdb.DuckDBPyConnection:
    # [Fix: Step 3] Create a real temporary database mimicking production explicitly
...
def test_endpoint_status_resolution(tmp_path):
    ...
    con.execute("INSERT INTO endpoint_state VALUES ('AAPL', 1, 'evt-1', '2025-01-01 10:00:00', '2025-01-01 09:00:00')")
    ...
    # [Fix: Step 4] Timestamp disambiguation assertions
    assert truth.last_success_age_s is not None
    assert truth.last_payload_change_age_s is not None
    assert truth.last_attempt_age_s is not None


def test_determinism_multi_ticker(tmp_path):
    """[Fix: Step 5] Asserts multi-ticker endpoint deterministically returns the latest success globally."""
    db_path = str(tmp_path / "test.db")
    con = setup_db(db_path)
    ...
    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {}}, db_path=db_path, validate_schema=False)
...
def test_variant_ordering_includes_failures(tmp_path):
    """[Fix: Step 5] Asserts variant resolution uses raw_http_events attempts if endpoint_state has no successes."""
    db_path = str(tmp_path / "test.db")
    con = setup_db(db_path)
    
    # Variant 1: Completely old failure
    con.execute("INSERT INTO dim_endpoints VALUES (1, 'GET', '/api/variant')")
    con.execute("INSERT INTO raw_http_events VALUES ('evt-v1', 1, '2025-01-01 10:00:00', 404, 'Not Found', NULL)")
    
    # Variant 2: Newer failure
    con.execute("INSERT INTO dim_endpoints VALUES (2, 'GET', '/api/variant')")
    con.execute("INSERT INTO raw_http_events VALUES ('evt-v2', 2, '2025-01-01 12:00:00', 500, 'Server Error', NULL)")

    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {}}, db_path=db_path, validate_schema=False)
    truth = checker.extract_db_truth(con, "GET", "/api/variant")
    
    assert truth.selected_endpoint_id == 2
    assert truth.last_status == 500
    assert truth.last_error == "Server Error"
...
def test_failure_visibility(tmp_path):
    ...
    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {"default": []}}, db_path=db_path, validate_schema=False)
...
def test_metric_computability_strictness(tmp_path):
    """Metric fails if required endpoint has no key rules and is NOT in presence_only_endpoints"""
    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {"default": []}}, db_path=str(tmp_path), validate_schema=False)

    ep = EndpointRef("GET", "/api/data")
    spec = MetricSpec(name="StrictMetric", required_endpoints=[ep], required_keys_by_endpoint={}, presence_only_endpoints=set())

    db_states = {ep: EndpointTruth(True, 200, None, 10, 10, 10, ["any"], 1, "evt-1")}
...
def test_metric_presence_only_logic(tmp_path):
    """Metric passes with COMPUTABLE_PRESENCE_ONLY flag if fallback is explicit"""
    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {"default": []}}, db_path=str(tmp_path), validate_schema=False)

    ep = EndpointRef("GET", "/api/data")
    spec = MetricSpec(name="PresenceMetric", required_endpoints=[ep], required_keys_by_endpoint={}, presence_only_endpoints={ep})
...
def test_metric_computability_logic(tmp_path):
    """Metric passes/fails correctly based on required keys"""
    checker = CapabilitiesChecker(catalog=DummyCatalog(), plan_yaml={"plans": {"default": []}}, db_path=str(tmp_path), validate_schema=False)

    ep = EndpointRef("GET", "/api/data")
    spec = MetricSpec(name="TestMetric", required_endpoints=[ep], required_keys_by_endpoint={ep: KeyRule(all_of={"req_1"})}, presence_only_endpoints=set())

    db_states_fail = {ep: EndpointTruth(True, 200, None, 10, 10, 10, ["wrong_key"], 1, "evt-1")}
    ...
    db_states_pass = {ep: EndpointTruth(True, 200, None, 10, 10, 10, ["req_1", "extra"], 1, "evt-1")}
