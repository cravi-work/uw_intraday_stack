...
def test_ingestion_dry_run_reaches_feature_insertion():
    """
    EVIDENCE: One-cycle ingestion dry run reaches feature insertion path.
    Proves the import graph (features <-> analytics) allows end-to-end execution
    ...
    with patch("src.ingest_engine.get_market_hours") as mock_gmh, \
        ...
        # Mock market hours to ensure we actually process the cycle (RTH)
        mock_mh = MagicMock()
        ...
        mock_db.writer.return_value.__enter__.return_value = mock_db_con
        ...
        # CRITICAL ASSERTION: Prove extract_all processed the data and reached insertion
